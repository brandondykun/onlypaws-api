...
# Depending on the environment, it will run the command in the dev, test, or staging environment

# Check if at least one argument is provided
if [ $# -lt 1 ]; then
    echo "Error: At least one argument is required"
    echo "You must specify either 'dev', 'test' or 'staging' environment as an argument"
    echo "Usage: ./assign_postimage_order.sh <dev|test|staging> [--dry-run]"
    exit 1
fi
